...
import { dynamicRoutes, notFoundAndNoPower } from './router'
 ...
 * @method setFilterMenuAndCacheTagsViewRoutes 设置递归过滤 有限权限的路由到 pinia routesList 中（已处理成多级嵌套路由）及缓存多级嵌套数组处理后的一维数组
 */
 ...
 * @link 参考： https://router.vuejs.org/zh/api/#addroute
 */
 ...
 * @description 替换 dynamicRoutes（/@/router/route）第一个顶级 children 路由
 * @returns 返回替换后的路由数组
 */
 ...
 * @param chil dynamicRoutes（/@/router/route）第一个顶级 children 下的路由集合
 * @returns 返回当前用户权限标识的路由数组
 */
 ...
 * @param roles 用户权限标识，在 userInfos（用户信息）的 roles（登陆页登录时缓存到浏览器）数组
