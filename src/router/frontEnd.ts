...
export function setFilterRouteEnd() {
	...
	if (filterRouteEnd[0] && filterRouteEnd[0].children) {
		const data = [...setFilterRoute(filterRouteEnd[0].children), ...notFoundAndNoPower]
		filterRouteEnd[0].children = data
	}
...
export function setCacheTagsViewRoutes() {
	...
	const data = formatTwoStageRoutes(formatFlatteningRoutes(rolesRoutes))[0]
	if (data && data.children) {
		storesTagsView.setTagsViewRoutes(data.children)
	}
}
...
export function setFilterMenuAndCacheTagsViewRoutes() {
	...
	const { userInfos } = storeToRefs(stores)
	if (dynamicRoutes[0] && dynamicRoutes[0].children) {
