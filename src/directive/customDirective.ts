...
/**
 * 自定义拖拽指令
 * @description 使用方式：v-drag="[dragDom, dragHeader]"，如：`<div v-drag="['.drag-container .el-diaglog', '.drag-container .el-dialog__header']"></div>`
 * @description dragDom 要拖动的元素，dragHeader 要拖动的 Header
 * @link 注意：https://github.com/element-plus/element-plus/issues/522
 * @link 参考：https://blog.csdn.net/weixin_46391323/article/details/105228020?utm_medium=distribute.pc_relevant.none-task-blog-baidujs_title-10&spm=1001.2101.3001.4242
 */
	...
	app.directive('drag', {
		mounted(el, binding) {
			...
			function down(e: any, type: string) {
				...
				// 注意在 ie 中，第一次取到的值为组件自带 50% 移动之后赋值为 px
				if (styL.inculdes('%')) {
			...
			/**
			 * 移动端
			 * ontouchstart 当按下手指时，触发 ontouchstart
			 * ontouchmove 当移动手指时，触发 ontouchmove
			 * ontouchend 当移走手指时，触发 ontouchend
			 */
